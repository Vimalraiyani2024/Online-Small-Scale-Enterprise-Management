...
if(isset($_SESSION['ossem_user_type'])&& isset($_SESSION['ossem_user_id']))
{
	...
	{	
		...
		$rs1=mysql_query("select a.sales_order_id,customer_name,address,city,state,pincode,mobile_no,phone_no,remark,reference,reference_date,tax,order_date,delivery_date from sales_order_master a,customer_master b where a.customer_id=b.customer_id and a.sales_order_id='$tmpid'",$con)or die(mysql_error());
		$row=mysql_fetch_array($rs1);	
?>
		<tr>
			<td colspan="4"><?php echo "<b id=b-title>Sales Order No. : ". $row[0]."</b>";?></td>
		</tr>
		<tr>				
			<td colspan="2" rowspan="5"><b id="b-title">Customer Detail: </b><br />
			<?php 
				echo strtoupper($row['customer_name'])."<br>".$row['address']."<br>".$row['city']." - ".$row['pincode']." , ".$row['state']."<br>Mobile No. : +91-".$row['mobile_no'];
				if(isset($row['phone_no']))
					echo'<br>Phone No.'.$row['phone_no'];
			?>
			</td>
			<td width="20%">Sales Date</td>
		...
		<tr> 				
			<td>Tax </td>
			<td><?php	$a=explode(",",$row['tax']); echo $a[2]." ".$a[0]." %";	?></td>
		</tr>				
		<tr>
			<Td colspan="4">
				...
				<?php 	
					...
					$sub_total=0.0;	
					$tmp=0.0;						
					$rs=mysql_query("select *from sales_order_detail where sales_order_id='$row[0]'",$con)or die(mysql_error());
