...
<form action="" method="post" role="form">
	...
	<h4 id="title">Contra Entry List</h4>
  <div class="table-responsive"> 
    <table class="table table-striped">
      <thead>
        <tr>
			<th style="text-align:right"><b id="b-title">Sr. No.</b></th>
          <th><b id="b-title">Contra Id</b></th>
	 ...
	 echo "<tr>";
	 $i=1;
	  $rs=mysql_query("select * from entry_master where entry_type='c' order by entry_id desc")or die(mysql_error());
	  while($row=mysql_fetch_array($rs))
	  {
	  	echo '<td style="text-align:right">'.$i++.'</td>';
	  	echo "<td width=8%>".str_pad($row['entry_id'],8,"0",STR_PAD_LEFT)."</td>";
