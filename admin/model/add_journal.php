				...
				<tbody>
				  <tr>
				   <tr>
					...
					<?php 
						$rs=mysql_query("select entry_id from  entry_master where entry_type='j' order by entry_id desc limit 1")or die(mysql_error());
					...
					<td colspan="3"><input type="text" class="form-control"  readonly="" id="datepicker1" name="entry_date" value="<?php echo date('d-m-Y');?>"placeholder="Ledger Name" required></td>
				  </tr>	
				  <tr>
				  ...
				  <td colspan="9">
							...
							<?php 
								...
								$rs=mysql_query("select * from ledger_master where group_id NOT IN (select group_id from group_master where group_name='Bank Account' or group_name='Cash On Hand')")or die(mysql_error());
				  ...
				  <td colspan="9">
					...
					<td colspan="6"><input class="form-control" name="remark" typej="text" id="remark" required/></td>
				 	</tr>
					<tr><td><input type="hidden" name="entry_type" id="entry_type" value="j" /></tr>
				</tbody>
      		</table>
			</div>			
			</div>
