...
if(isset($_REQUEST['tmpid']))
{
	...
	$list=$_REQUEST['list'];
	$rs1=mysql_query("select a.pur_return_id,pur_return_date,supplier_name,address,city,state,pincode,mobile_no,phone_no,reference,ref_date,tax,despatch_by,despatch_date,remark from pur_return a,supplier_master b where a.supplier_id=b.supplier_id and a.pur_return_id='$tmpid'")or die(mysql_error());
...
<form action="" method="post" id="form1">	
			<div id="btn-header">		
			<a href="purchase.php?close=c_pur_return" id="close-box">Close</a>
			<a href="purchase.php?print=purchase_return&id=<?php echo $row[0]; ?>" id="btn-print">Print</a>
			<a href="#purchase.php?piid='.$row['pur_return_id'].'&list=pur_return_del" class="btn-delete">Cancel</a>
			...
			<tbody>
			  <tr>
				<td colspan="4"><?php echo "<h4>Purchase Return Invoice No. : ". strtoupper($row[0])."</h4>";?></td>
			</tr>
			<tr>
				...
				<td width="20%">Purchase Return Date</td>
				<td><?php echo date("d-m-Y",strtotime($row['pur_return_date'])); ?></td>			
			</tr>
			<tr>					
			...
			<?php
				if($row["despatch_by"]!="" && $row["despatch_by"]!="")
					...
					echo'<tr><td>despatch Date </td><td>'.$row['despatch_date'].'</td></tr>';
			?>				
			<tr>
				<Td colspan="4">
					...
					<tr>
						<?php 	
							...
							$rs=mysql_query("select *from pur_return_detail where pur_return_id='$row[0]'")or die(mysql_error());
